...
describe('executed: an unmet runtime requirement keeps its provenance in routed mode', () => {
  ...
  // ahead of it, so an earlier unmet gate — a missing controller, or an
  // unknown forward-compat value — still wins.
  test('an unrecognized requirement wins in both modes, in either declared order', async () => {
    // The routed `request_signer` verdict must never be consumed while the
    // storyboard names a gate this SDK cannot interpret: the run has not
    // assessed that gate, and a non-routed run reports it because its own
    // `request_signer` case self-skips without a profile.
    ...
    try {
      for (const requires of [
        ['request_signer', 'future_runtime'],
        ['future_runtime', 'request_signer'],
      ]) {
        const storyboard = {
        ...
        const reported = result =>
          result.phases
            ...
            .find(Boolean);
        assert.equal(reported(nonRouted), 'future_runtime', `${requires.join(',')} (non-routed)`);
        assert.equal(reported(routed), 'future_runtime', `${requires.join(',')} (routed)`);
      }
    } finally {
      await closeConnections();
      await agent.close();
...
describe('requirement reporting is identical routed and non-routed', () => {
  ...
  // varies by mode is only which gates are assessable at that point, and the
  // runner now arranges for both modes to see the same ones:
  //
  //   - no root capability predicate: both modes run the full list
  //     pre-discovery. `controller` / `request_signer` self-defer without a
  //     profile; `multi_agent` and unrecognized values answer in place.
  //   - with a root predicate: both modes defer the whole list until after
  //     capability discovery, and the routed `request_signer` verdict is
  //     consumed at its declared position.
  //
  // `origin/main` diverges on four of the ten cells below — the two
  // `['controller', …]` no-predicate rows (routed answered `controller`
  // post-discovery where non-routed had already fallen through to the next
  // declared gate) and both satisfied-predicate `request_signer` rows (routed
  // reported `capability_unsupported`, mislabeling an unmet requirement as an
  // unclaimed capability). Every cell agrees here.
  const CASES = [
    [false, ['request_signer', 'future_runtime'], 'future_runtime'],
    [false, ['future_runtime', 'request_signer'], 'future_runtime'],
    [false, ['multi_agent', 'request_signer', 'future_runtime'], 'multi_agent'],
    [false, ['controller', 'future_runtime'], 'future_runtime'],
    [false, ['controller', 'multi_agent'], 'multi_agent'],
