...
function parseFlags(argv) {
  ...
  return parseRequestSigningFlags(readFlag, hasFlag);
}
...
test('a coverage gap renders as COVERAGE UNAVAILABLE with its remedy; an inapplicable step does not', () => {
  ...
  // nothing else reads as a clean pass. `fixture_unavailable` is the canonical
  // reason the A2A vector gap carries (see request-signing-vector-transport
  // tests), and it has to look different from an ordinary skip.
  ...
  const gap = formatStepSkipLines({
    ...
    skip: {
      ...
      detail: 'Coverage unavailable: … Remedy: grade the MCP binding, or set `--signing-transport`.',
    },
  });
  assert.deepStrictEqual(gap, [
    '   COVERAGE UNAVAILABLE (fixture_unavailable / signing_transport_unavailable)',
...
test('a single-step run renders an unavailable step as not verified, never as Passed', () => {
  ...
  const unavailable = formatStepVerdictLines(
    {
      ...
      passed: true,
      skip_reason: 'signing_transport_unavailable',
      skip: { reason: 'fixture_unavailable', detail: 'Coverage unavailable: … Remedy: grade the MCP binding.' },
    },
    { durationMs: 12 }
  );
  ...
  assert.match(unavailable[1], /COVERAGE UNAVAILABLE \(fixture_unavailable \/ signing_transport_unavailable\)/);
