...
| `security_baseline` reports `auth_mechanism_verified: []` | Your read surface is outside the probe allowlist, so the runner falls back to the MCP session probe (step task shows as `mcp_session_probe`). It can only earn the contribution on the **OAuth/PRM** branch — serve correct RFC 9728 metadata and run the storyboard with OAuth (`--oauth`). The static API-key and Basic branches gate on a positive probe that asserts an AdCP response body, which no protocol operation produces, so the durable fix is to advertise one allowlisted read tool (`list_creatives`, `get_signals`, `list_accounts`, …) |
| An `mcp_session_probe` step reports `inconclusive` | A rejection is only auth evidence when a valid credential of the **same kind** completes the same lifecycle. The message names which: no OAuth access token for this run (run with `--oauth`), no API key / Basic credential configured, or the valid credential was refused too (refresh it and confirm the agent URL is reachable) |
| An `mcp_session_probe` step fails with a 200 | The probe reached `tools/list` unauthenticated or with a bogus credential and got a success — the agent is serving protected MCP operations to anyone. `extraction.note` names the stage the verdict landed on (`initialize` vs `tools/list`) |
