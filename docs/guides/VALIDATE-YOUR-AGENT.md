...
| `None of the required contributions were recorded: ["auth_mechanism_verified"]` | No auth mechanism could be verified. If your steps also show task `mcp_session_probe`, your read surface is outside the probe allowlist and the runner fell back to the MCP session probe — see the next two rows |
| `[session probe cannot grade]` on a step, with `Skipped: … cannot grade a positive static-credential step` | **A static-credential-only agent with no allowlisted read tool cannot be certified.** The positive API-key/Basic probe asserts an AdCP task response body (`context`, `context.correlation_id`) that no MCP protocol operation produces, so its branch's contribution gate stays closed. Two remedies: advertise one allowlisted read tool (`list_creatives`, `get_media_buy_delivery`, `get_signals`, `list_accounts`, …) — the durable fix — or serve RFC 9728 metadata and run with `--oauth` so the OAuth branch is verified instead. Making the static branch verifiable needs a session-layer positive step in the storyboard itself: [adcontextprotocol/adcp#2940](https://github.com/adcontextprotocol/adcp-client/issues/2940) |
| `Error: MCP session auth probe is inconclusive: …` | A rejection is auth evidence only when a valid credential of the **same kind** completes the same lifecycle. The message names which is missing: no OAuth access token for this run (run with `--oauth`), no API key / Basic credential configured, or the valid credential was refused too (refresh it and confirm the agent URL is reachable) |
| `Error: MCP session auth probe refuses this as auth evidence: …` | The probe reached `tools/list` unauthenticated or with a bogus credential and got a success — the agent is serving protected MCP operations to anyone. `extraction.note` names the stage and verdict (`initialize` vs `tools/list`) |
